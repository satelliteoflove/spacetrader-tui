...
func RefreshSystemPrices(gs *GameState, sysIdx int) {
	...
	for g, good := range gs.Data.Goods {
		if good.MinTech <= sys.TechLevel && sys.TechLevel <= good.MaxTech {
			gs.Systems[sysIdx].Prices[g] = calculateBasePrice(good, sys, event, gs.Rand)
		} else {
...
func calculateBasePrice(good gamedata.GoodDef, sys gamedata.SystemDef, event string, rng *rand.Rand) int {
	...
	if sys.Resource.String() == good.ExpensiveResource && good.ExpensiveResource != "" {
		price = price * 150 / 100
	}
	if sys.Resource.String() == good.CheapResource && good.CheapResource != "" {
		price = price * 70 / 100
	}

	if (sys.PoliticalSystem == gamedata.PolDictatorship || sys.PoliticalSystem == gamedata.PolFascist) && !good.Legal {
		price = price * 150 / 100
	}
